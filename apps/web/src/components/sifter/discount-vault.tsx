...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@turbo/ui/tabs";
  ...
  <Sheet>
    ...
    <SheetContent className="w-full sm:max-w-md">
      ...
      <ScrollArea className="min-h-0 flex-1 px-6 pb-6">
        <Tabs defaultValue="temu" className="min-h-0 gap-5">
          <TabsList className="grid h-10 w-full grid-cols-2 rounded-full p-1">
            <TabsTrigger
              value="temu"
              className="rounded-full data-[state=active]:bg-white data-[state=active]:text-slate-950 data-[state=active]:shadow-sm dark:data-[state=active]:bg-white dark:data-[state=active]:text-slate-950"
            >
              Temu
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="temu" className="mt-0">
            <div className="space-y-7">
              {SIFTER_TEMU_COUPON_GROUPS.map((group) => (
